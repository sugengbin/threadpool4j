...
import cn.aofeng.threadpool4j.common.DefaultThreadFactory;
import cn.aofeng.threadpool4j.common.IThreadPoolLifeCycle;
import cn.aofeng.threadpool4j.executor.CounterThreadPoolExecutor;
 ...
 */
public class ThreadPoolImpl implements IThreadPoolLifeCycle, ThreadPool {
	...
	public void init() {
		if (ThreadPoolStatus.UNINITIALIZED != _status) {
			_logger.warn(
					"initialization thread pool failed, because the status was wrong, current status was {} (0:UNINITIALIZED, 1:INITIALITION_SUCCESSFUL, 2:INITIALITION_FAILED, 3:DESTROYED)",
					_status);
			return;
		}
	...
	private void initThreadPool() {
		...
		if (!_threadPoolConfig.containsPool(DEFAULT_THREAD_POOL)) {
			throw new IllegalStateException(
					String.format("the default thread pool not exists, please check the config file '%s'",
							_threadPoolConfig._configFile));
		}
		...
		for (ThreadPoolInfo threadPoolInfo : threadPoolInfoList) {
			...
			CounterThreadPoolExecutor threadPool = new CounterThreadPoolExecutor(threadPoolInfo.getCoreSize(),
					threadPoolInfo.getMaxSize(), threadPoolInfo.getThreadKeepAliveTime(), TimeUnit.SECONDS, workQueue,
	...
	private void startThreadPoolStateJob() {
		...
		_threadPoolStateJob = new ThreadPoolStateJob(_multiThreadPool, _threadPoolConfig.getThreadPoolStateInterval());
	...
	@Override
	public int getSubmittedTaskCount(String threadpoolName) {
		ExecutorService service = getThreadPool(threadpoolName);
		int count = 0;
		if (service != null) {
			CounterThreadPoolExecutor executor = (CounterThreadPoolExecutor) service;
			count = executor.getSubmittedTaskCount();
		}
		return count;
	}
